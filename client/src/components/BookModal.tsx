...
export default function BookModal(props: ModalProps) {
  ...
  const handle_submit = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (props.action === 'edit' && props.book_id) {
      let { success, detail } = await updateBook({ ...data, id: props.book_id })
      if (success) {
  ...
  useEffect(() => {
    if (props.action === 'edit' && props.book_id) {
      getBookData(props.book_id)
    }
  }, [])

  return (
    <Modal id={props.id} onClose={props.onClose}
      className="md:max-h-5/6 mb-auto md:my-auto md:w-3/4 lg:w-1/3 mx-auto px-8 py-4 bg-gray-200">
      <div
        onClick={(e) => {
          e.preventDefault()
